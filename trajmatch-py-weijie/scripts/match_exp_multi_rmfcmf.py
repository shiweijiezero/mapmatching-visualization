...
def match_one(fname, vid, gps_java_arg, mee_java_arg, opt, return_dict):
    ...
    try:
        ...
        if (len(mees_after_weighted_mean[0]) < 3):
            ...
            return
        gps_result_rids = netmatch(traj_str=str(lines_after_interval_with_ts), java_args=gps_java_arg, opt=opt, url="http://127.0.0.1:8090/netmatch")
        mee_result_rids = netmatch(traj_str=str(mees_after_weighted_mean), java_args=mee_java_arg, opt=opt, url="http://127.0.0.1:8093/netmatch")
        ...
        cmf = CMF(gps_matched_lines, mee_matched_lines,corridor_width=30)
        if rmf > 2:
            rmf = -1
            cmf = -1
        with open(fname, mode='a', encoding='utf-8') as f:
            if len(gps_result_rids) > 0:
                info = str(vid)+'\tmee\t'+str(mee_result_rids)+'\n'
                info += str(vid)+'\tgps\t'+str(gps_result_rids)+'\n'
                info += 'rmf\t'+str(rmf)
                f.write(info + '\n')
            # info = f'{vid}\t{"%.3f" % rmf}\t{"%.3f" % cmf}'
            # f.write(info + '\n')
        print("[Result]", info)
...
def match_all(kwargs):
    ...
    rmf_values = [x[0] for x in list(return_dict.values())]
    cmf_values = [x[1] for x in list(return_dict.values())]
    rmf_values = list(filter(lambda x: x != -1, rmf_values))
    cmf_values = list(filter(lambda x: x != -1, cmf_values))
    rmf = sum(rmf_values)
    cmf = sum(cmf_values)
